...
def post_process(df, date) -> pd.DataFrame:
    ...
    df["LastBestBidVolume"] = df["LastBestBidVolume"].str.replace("", "0").str.replace(",", "").str.replace("--", "0").astype(int)
    df["LastBestAskPrice"] = df["LastBestAskPrice"].str.replace(",", "").str.replace("--", "0").astype(float)
    df["LastBestAskVolume"] = df["LastBestAskVolume"].str.replace("", "0").str.replace(",", "").str.replace("--", "0").astype(int)
