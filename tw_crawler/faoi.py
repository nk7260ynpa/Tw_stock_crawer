...
def remove_comma(x) -> str:
    ...
    部分自營商 hedge 欄位於休市或無交易時可能為 None/NaN，
...
def post_process(df: pd.DataFrame, date: str) -> pd.DataFrame:
    """將從 FAOI 網站爬取的原始資料表做欄位轉換與清洗。

    Args:
        df: 從 FAOI 網站爬取的原始 DataFrame。
    ...
    df["ForeignInvestorsTotalBuy"] = (
        df["ForeignInvestorsTotalBuy"].map(remove_comma).astype(int)
    )
    df["ForeignInvestorsTotalSell"] = (
        df["ForeignInvestorsTotalSell"].map(remove_comma).astype(int)
    )
    df["ForeignInvestorsDifference"] = (
        df["ForeignInvestorsDifference"].map(remove_comma).astype(int)
    )
    df["ForeignDealersTotalBuy"] = (
        df["ForeignDealersTotalBuy"]
        .astype(str).map(remove_comma).astype(int)
    )
    df["ForeignDealersTotalSell"] = (
        df["ForeignDealersTotalSell"]
        .astype(str).map(remove_comma).astype(int)
    )
    df["ForeignDealersDifference"] = (
        df["ForeignDealersDifference"]
        .astype(str).map(remove_comma).astype(int)
    )
    df["SecuritiesInvestmentTotalBuy"] = (
        df["SecuritiesInvestmentTotalBuy"].map(remove_comma).astype(int)
    )
    df["SecuritiesInvestmentTotalSell"] = (
        df["SecuritiesInvestmentTotalSell"].map(remove_comma).astype(int)
    )
    df["SecuritiesInvestmentDifference"] = (
        df["SecuritiesInvestmentDifference"].map(remove_comma).astype(int)
    )
    df["DealersDifference"] = (
        df["DealersDifference"].map(remove_comma).astype(int)
    )
    df["DealersProprietaryTotalBuy"] = (
        df["DealersProprietaryTotalBuy"].map(remove_comma).astype(int)
    )
    df["DealersProprietaryTotalSell"] = (
        df["DealersProprietaryTotalSell"].map(remove_comma).astype(int)
    )
    df["DealersProprietaryDifference"] = (
        df["DealersProprietaryDifference"].map(remove_comma).astype(int)
    )
    df["DealersHedgeTotalBuy"] = (
        df["DealersHedgeTotalBuy"]
        .astype(object).fillna("0").astype(str)
        .map(remove_comma).astype(float).astype(int)
    )
    df["DealersHedgeTotalSell"] = (
        df["DealersHedgeTotalSell"]
        .astype(object).fillna("0").astype(str)
        .map(remove_comma).astype(float).astype(int)
    )
    df["DealersHedgeDifference"] = (
        df["DealersHedgeDifference"]
        .astype(object).fillna("0").astype(str)
        .map(remove_comma).astype(float).astype(int)
    )
    df["TotalDifference"] = (
        df["TotalDifference"].map(remove_comma).astype(int)
    )
